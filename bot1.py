...
# ---------------- SETTINGS ----------------
TOKEN = "8411732632:AAG4r***************************"
OWNER_ID = 7328238543

BASE_DIR = r"C:\Users\server\Desktop\тест-хост"
DATA_DIR = r"C:\Users\server\Desktop\тест-хост\data"
FILES_DIR = r"C:\Users\server\Desktop\тест-хост\files"
QUARANTINE_DIR = r"C:\Users\server\Desktop\тест-хост\quarantine"
DB_PATH = r"C:\Users\server\Desktop\тест-хост\db.sqlite3"

# Лимит размера файла для всех тарифов (10 МБ)
...
FORBIDDEN_PYTHON_PATTERNS: Set[str] = {
    "exec(", "eval(", "os.system", "memory.append", "subprocess.popen", "base64.b64decode",
...
EDIT_FLOW = {}     # {user_id: {"file_path": str, "abs_path": str}}
NEW_FILE_FLOW = {} # {user_id: {"current_dir": str}}
NEW_FOLDER_FLOW = {} # {user_id: {"current_dir": str}}
    ...
    def update_process_stats(self):
        ...
        for key, stats in list(self.process_stats.items()):
            ...
            # 2. Проверка суммарных лимитов пользователя
                ...
                reason = f"Суммарный лимит CPU: {total_cpu:.1f}%/{user_total_cpu_percent}%"
...
def delete_user_files(uid: int):
    ...
    # Удаляем файлы пользователя из карантина
    with db_connect() as conn:
        # Получаем список файлов в карантине для этого пользователя
        quarantined = conn.execute("SELECT qid, saved_path FROM quarantine WHERE user_id=?", (uid,)).fetchall()
        for q_file in quarantined:
            # Удаляем физический файл в карантине
            if os.path.exists(q_file["saved_path"]):
                try:
                    os.remove(q_file["saved_path"])
                except Exception as e:
                    print(f"Ошибка удаления файла из карантина: {e}")
        # Удаляем записи из карантина
        conn.execute("DELETE FROM quarantine WHERE user_id=?", (uid,))
        conn.commit()
...
def set_ban(uid: int, val: int):
    ...
    # При бане удаляем все файлы пользователя
    if val == 1:
        delete_user_files(uid)
        # Уведомляем пользователя
        try:
            bot.send_message(uid, "🚫 Ваш аккаунт заблокирован. Все файлы удалены.")
        except Exception:
            pass
...
def handle_upload(msg):
    ...
    if msg.document.file_size > MAX_FILE_SIZE:
        ...
        return bot.reply_to(
            msg, 
            f"⚠️ Размер файла превышает лимит!\n"
            f"📁 Ваш файл: {size_mb:.1f} МБ\n"
            f"📏 Лимит: {max_size_mb} МБ\n\n"
            f"Уменьшите размер файла и попробуйте снова."
        )

    # сохраняем файл
    file_info = bot.get_file(msg.document.file_id)
    
    # Дополнительная проверка размера через file_info
    if hasattr(file_info, 'file_size') and file_info.file_size and file_info.file_size > MAX_FILE_SIZE:
    ...
    # Финальная проверка размера скачанных данных
    if len(downloaded) > MAX_FILE_SIZE:
        size_mb = len(downloaded) / (1024 * 1024)
        max_size_mb = MAX_FILE_SIZE / (1024 * 1024)
        return bot.reply_to(
            msg, 
            f"⚠️ Размер файла превышает лимит!\n"
...
def explorer_view(uid: int, relative_path: str) -> Tuple[str, types.InlineKeyboardMarkup]:
    ...
    for entry in files:
        ...
        # Создаем ряд с файлом и кнопками действий
        row_buttons = []
        row_buttons.append(types.InlineKeyboardButton(f"📄 {entry}", callback_data=f"explorer_file:{encoded}"))
        row_buttons.append(types.InlineKeyboardButton("✏️", callback_data=f"explorer_edit:{encoded}"))
        row_buttons.append(types.InlineKeyboardButton("🗑️", callback_data=f"explorer_del_confirm:{encoded}"))
        kb.add(*row_buttons)

    # Кнопки создания файлов и папок
    encoded_current = urllib.parse.quote_plus(rel_path)
    kb.add(
        types.InlineKeyboardButton("📝 Создать файл", callback_data=f"explorer_new_file:{encoded_current}"),
        types.InlineKeyboardButton("📁 Создать папку", callback_data=f"explorer_new_folder:{encoded_current}")
    )
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("explorer_edit:"))
def explorer_edit_file(c):
    uid = c.from_user.id
    u = get_user(uid)
    if u["plan"] == "free":
        return bot.answer_callback_query(c.id, "✏️ Редактирование доступно только на платных тарифах.", show_alert=True)
    ...
    if os.path.isdir(abs_path):
        return bot.answer_callback_query(c.id, "❌ Нельзя редактировать папку", show_alert=True)

    # Проверяем размер файла
    file_size = os.path.getsize(abs_path)
    if file_size > 50 * 1024:  # 50KB лимит
        return bot.answer_callback_query(c.id, "❌ Файл слишком большой для редактирования", show_alert=True)

    try:
        with open(abs_path, "r", encoding="utf-8") as f:
            content = f.read()
        
        # Сохраняем в состоянии редактирования
        EDIT_FLOW[uid] = {"file_path": rel_norm, "abs_path": abs_path}
        
        bot.send_message(
            uid,
            f"✏️ <b>Редактирование файла:</b> <code>/{rel_norm}</code>\n\n"
            f"Отправьте новый текст файла:\n\n"
            f"<code>{content[:1000] + '...' if len(content) > 1000 else content}</code>",
            parse_mode="HTML"
        )
        bot.answer_callback_query(c.id, "✏️ Редактируйте файл")
        
    except Exception as e:
        bot.answer_callback_query(c.id, f"❌ Ошибка чтения: {e}", show_alert=True)

@bot.callback_query_handler(func=lambda c: c.data.startswith("explorer_del_confirm:"))
def explorer_delete_confirm(c):
    uid = c.from_user.id
    raw_path = c.data.split(":", 1)[1] if ":" in c.data else ""
    rel_path = urllib.parse.unquote_plus(raw_path)
    ...
    kb = types.InlineKeyboardMarkup()
    kb.add(
        types.InlineKeyboardButton("✅ Да, удалить", callback_data=f"explorer_del:{urllib.parse.quote_plus(rel_norm)}"),
        types.InlineKeyboardButton("❌ Отмена", callback_data=f"explorer:{urllib.parse.quote_plus(os.path.dirname(rel_norm))}")
    )
    
    bot.edit_message_text(
        f"🗑️ <b>Подтверждение удаления</b>\n\n"
        f"Вы уверены, что хотите удалить файл:\n"
        f"<code>/{rel_norm}</code>?",
        c.message.chat.id,
        c.message.id,
        reply_markup=kb,
        parse_mode="HTML"
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("explorer_del:"))
def explorer_delete_file(c):
    uid = c.from_user.id
    u = get_user(uid)
    if u["plan"] == "free":
        return bot.answer_callback_query(c.id, "🗂 Проводник доступен только на платных тарифах.", show_alert=True)
    ...
    if os.path.isdir(abs_path):
        return bot.answer_callback_query(c.id, "❌ Нельзя удалить папку через это меню", show_alert=True)

    try:
        # Если это .py или .js файл, удаляем запись из базы
        if rel_norm.endswith(('.py', '.js')):
            remove_file_record(uid, rel_norm)
            
            # Останавливаем скрипт если он запущен
            key = (uid, os.path.basename(rel_norm))
            if key in RUNNING:
                stop_script_for_user(uid, os.path.basename(rel_norm))
        
        # Удаляем физический файл
        os.remove(abs_path)
        
        # Возвращаемся в текущую папку
        parent_dir = os.path.dirname(rel_norm)
        text, kb = explorer_view(uid, parent_dir)
        safe_edit_text(bot.edit_message_text, text, c.message.chat.id, c.message.id, reply_markup=kb, parse_mode="HTML")
        
        bot.answer_callback_query(c.id, "🗑️ Файл удален")
        
    except OSError as exc:
        bot.answer_callback_query(c.id, f"❌ Ошибка удаления: {exc}", show_alert=True)

@bot.callback_query_handler(func=lambda c: c.data.startswith("explorer_new_file:"))
def explorer_new_file(c):
    uid = c.from_user.id
    u = get_user(uid)
    if u["plan"] == "free":
        return bot.answer_callback_query(c.id, "📝 Создание файлов доступно только на платных тарифах.", show_alert=True)

    raw_path = c.data.split(":", 1)[1] if ":" in c.data else ""
    current_dir = urllib.parse.unquote_plus(raw_path)

    NEW_FILE_FLOW[uid] = {"current_dir": current_dir}
    
    bot.send_message(
        uid,
        f"📝 <b>Создание нового файла</b>\n\n"
        f"Текущая папка: <code>/{current_dir if current_dir else ''}</code>\n\n"
        f"Введите имя нового файла (с расширением .py, .js, .txt и т.д.):",
        parse_mode="HTML"
    )
    bot.answer_callback_query(c.id, "📝 Создание файла")

@bot.callback_query_handler(func=lambda c: c.data.startswith("explorer_new_folder:"))
def explorer_new_folder(c):
    uid = c.from_user.id
    u = get_user(uid)
    if u["plan"] == "free":
        return bot.answer_callback_query(c.id, "📁 Создание папок доступно только на платных тарифах.", show_alert=True)

    raw_path = c.data.split(":", 1)[1] if ":" in c.data else ""
    current_dir = urllib.parse.unquote_plus(raw_path)

    NEW_FOLDER_FLOW[uid] = {"current_dir": current_dir}
    
    bot.send_message(
        uid,
        f"📁 <b>Создание новой папки</b>\n\n"
        f"Текущая папка: <code>/{current_dir if current_dir else ''}</code>\n\n"
        f"Введите имя новой папки:",
        parse_mode="HTML"
    )
    bot.answer_callback_query(c.id, "📁 Создание папки")
...
# ---------------- FILE EDITING ----------------
@bot.message_handler(func=lambda m: m.from_user.id in EDIT_FLOW)
def handle_file_edit(msg):
    uid = msg.from_user.id
    flow_data = EDIT_FLOW.get(uid)
    if not flow_data:
        return

    new_content = msg.text
    file_path = flow_data["file_path"]
    abs_path = flow_data["abs_path"]

    try:
        with open(abs_path, "w", encoding="utf-8") as f:
            f.write(new_content)
        
        # Если это .py или .js файл, обновляем импорты в базе
        if file_path.endswith(('.py', '.js')):
            ...
            with db_connect() as conn:
                conn.execute(
                    "UPDATE files SET imports=? WHERE user_id=? AND filename=?",
                    (",".join(imports), uid, file_path)
                )
                conn.commit()

        bot.reply_to(msg, f"✅ Файл <code>{file_path}</code> успешно обновлен!", parse_mode="HTML")
        
    except Exception as e:
        bot.reply_to(msg, f"❌ Ошибка сохранения: {e}")

    EDIT_FLOW.pop(uid, None)

# ---------------- FILE CREATION ----------------
@bot.message_handler(func=lambda m: m.from_user.id in NEW_FILE_FLOW)
def handle_new_file(msg):
    uid = msg.from_user.id
    flow_data = NEW_FILE_FLOW.get(uid)
    if not flow_data:
        return

    filename = msg.text.strip()
    current_dir = flow_data["current_dir"]

    # Проверяем расширение
    if not any(filename.endswith(ext) for ext in ['.py', '.js', '.txt', '.md', '.json', '.html', '.css']):
        bot.reply_to(msg, "❌ Поддерживаются только файлы .py, .js, .txt, .md, .json, .html, .css")
        NEW_FILE_FLOW.pop(uid, None)
        return

    try:
        # Формируем полный путь
        full_path = os.path.join(current_dir, filename) if current_dir else filename
        _, rel_norm, abs_path = resolve_user_path(uid, full_path)

        # Создаем файл
        with open(abs_path, "w", encoding="utf-8") as f:
            if filename.endswith('.py'):
                f.write('# Ваш Python код здесь\nprint("Hello World!")')
            elif filename.endswith('.js'):
                f.write('// Ваш JavaScript код здесь\nconsole.log("Hello World!");')
            else:
                f.write('')  # Пустой файл

        # Если это скрипт, проверяем его содержимое
        if filename.endswith(('.py', '.js')):
            with open(abs_path, "r", encoding="utf-8") as f:
                code = f.read()

            file_ext = '.py' if filename.endswith('.py') else '.js'
            if file_ext == '.py':
                imports = extract_python_imports(code)
            else:
                imports = extract_js_requires(code)
            ...
            if bad and not u["protection_disabled"]:
                ...
                qpath = quarantine_path(rel_norm)
                os.rename(abs_path, qpath)
                qid = add_quarantine(uid, rel_norm, imports, qpath)

                bot.reply_to(msg, f"🚫 Файл содержит запрещённые конструкции: {', '.join(bads)}")
                bot.send_message(OWNER_ID,
                    f"⚠️ Пользователь <a href='tg://user?id={uid}'>{uid}</a> "
                    f"создал файл с опасным кодом.\n"
                    f"<b>Файл:</b> {rel_norm}\n"
                    f"<b>Тип:</b> {file_ext}\n"
                    f"<b>Обнаружено:</b> {', '.join(bads)}",
                    parse_mode='HTML',
            ...
            # Если проверку прошли, добавляем в базу
            add_file_record(uid, rel_norm, imports)

        bot.reply_to(msg, f"✅ Файл <code>{filename}</code> создан!", parse_mode="HTML")
        
        # Показываем обновленный проводник
        text, kb = explorer_view(uid, current_dir)
        bot.send_message(uid, text, reply_markup=kb, parse_mode="HTML")
        
    except Exception as e:
        bot.reply_to(msg, f"❌ Ошибка создания файла: {e}")

    NEW_FILE_FLOW.pop(uid, None)

# ---------------- FOLDER CREATION ----------------
@bot.message_handler(func=lambda m: m.from_user.id in NEW_FOLDER_FLOW)
def handle_new_folder(msg):
    uid = msg.from_user.id
    flow_data = NEW_FOLDER_FLOW.get(uid)
    if not flow_data:
        return
    ...
    try:
        # Формируем полный путь
        full_path = os.path.join(current_dir, folder_name) if current_dir else folder_name
        _, rel_norm, abs_path = resolve_user_path(uid, full_path)

        # Создаем папку
        os.makedirs(abs_path, exist_ok=True)

        bot.reply_to(msg, f"✅ Папка <code>{folder_name}</code> создана!", parse_mode="HTML")
        ...
        bot.reply_to(msg, f"❌ Ошибка создания папки: {e}")

    NEW_FOLDER_FLOW.pop(uid, None)
...
def admin_kb():
    ...
    kb.add(
        ...
        types.InlineKeyboardButton("🧹 Очистить файлы", callback_data="adm_cleanuser")
    )
    kb.add(types.InlineKeyboardButton("⬅️ Назад", callback_data="back"))
    return kb
...
@bot.callback_query_handler(func=lambda c: c.data == "adm_cleanuser")
def adm_cleanuser_cb(c):
    if c.from_user.id != OWNER_ID:
        return
    ADMIN_FLOW[c.from_user.id] = {"mode": "clean_user"}
    bot.send_message(c.message.chat.id, "🧹 Введи user_id для очистки файлов (без бана):")
...
def admin_flow_handler(m):
    ...
    elif mode == "clean_user":
        try:
            target = int(m.text.strip())
            delete_user_files(target)
            bot.reply_to(m, f"🧹 Файлы пользователя {target} полностью очищены")
            ADMIN_FLOW.pop(m.from_user.id, None)
            
            # Уведомляем пользователя (если не забанен)
            u = get_user(target)
            if not u["banned"]:
                bot.send_message(target, "🧹 Все ваши файлы были очищены администратором.")
        except ValueError:
            bot.reply_to(m, "❌ Неверный user_id. Введите число.")
...
def adm_ban_callback(c):
    ...
    set_ban(uid, 1)
    bot.answer_callback_query(c.id, f"Пользователь {uid} заблокирован и все файлы удалены")
    bot.edit_message_reply_markup(c.message.chat.id, c.message.id, reply_markup=None)
    
    # Уведомляем админа о завершении
    bot.send_message(OWNER_ID, f"✅ Пользователь {uid} заблокирован, все файлы удалены")
...
def adm_ignore_callback(c):
    ...
    if q_file and os.path.exists(q_file["saved_path"]):
        try:
            os.remove(q_file["saved_path"])
        except Exception as e:
            print(f"Ошибка удаления файла из карантина: {e}")
    ...
    with db_connect() as conn:
        conn.execute("DELETE FROM quarantine WHERE qid=?", (qid,))
        conn.commit()
    
    bot.answer_callback_query(c.id, "Файл отклонен и удален")
    bot.edit_message_reply_markup(c.message.chat.id, c.message.id, reply_markup=None)
    ...
    if q_file:
        ...
        bot.send_message(uid,
                         f"🚫 Ваш файл <b>{filename}</b> не прошёл проверку администратора и был удалён.",
                         parse_mode="HTML")
...
def adm_approve_callback(c):
    ...
    if approved_file:
        ...
        bot.send_message(uid,
                         f"✅ Ваш файл <b>{filename}</b> был проверен администратором и разрешен!",
                         parse_mode="HTML")
        
        # Если файл был создан через проводник, показываем обновленный проводник
        try:
...
def on_start(msg):
    ...
    text = (f"🏠 <b>Главное меню</b>\n"
            ...
            f"📏 Макс. размер файла: {max_size_mb} MB\n"
...
def ask_upload(c):
    ...
    info_text = (
        ...
        f"• 🐍 <code>.py</code> - Python скрипты\n"
        f"• 📜 <code>.js</code> - JavaScript скрипты\n"
        f"• 📦 <code>.zip</code> - Архивы с файлами\n\n"
        f"📏 <b>Максимальный размер файла:</b> {max_size_mb} МБ\n\n"
        f"Отправьте файл для загрузки:"
    )
    
    bot.answer_callback_query(c.id, f"📤 Отправьте .py, .js или .zip файл (до {max_size_mb} МБ)")
...
def buy_plan(c):
    ...
    plans_text = (
        ...
        f"📏 <b>Общий лимит для всех тарифов:</b> макс. размер файла {max_size_mb} МБ\n\n"
        f"🪶 <b>Бесплатный</b>\n"
        f"• Файлов: 2\n"
        f"• RAM на файл: 512 MB | Суммарно: 800 MB\n"
        f"• CPU на файл: 5% | Суммарно: 8%\n"
        f"• Срок: бессрочно\n\n"
        f"💎 <b>Премиум</b>\n"
        f"• Файлов: 20\n"
        f"• RAM на файл: 1024 MB | Суммарно: 2048 MB\n"
        f"• CPU на файл: 15% | Суммарно: 25%\n"
        f"• Срок: по выбору\n\n"
        f"🚀 <b>Максимальный</b>\n"
        f"• Файлов: 50\n"
        f"• RAM на файл: 2048 MB | Суммарно: 4096 MB\n"
        f"• CPU на файл: 30% | Суммарно: 50%\n"
        f"• Срок: по выбору\n\n"
        f"Для покупки обратитесь: @Arkadarootfurry\n\n"
        f"Просим обратить внимание!!!\n\n"
        f"Бот работает на домашнем сервере! В скором времени он будет перенесён на хостинг."
    )
